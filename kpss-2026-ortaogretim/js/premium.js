...
(function(){
  ...
  function enhanceStudy(){
    ...
    $("#studyFocus")?.addEventListener('click',()=>{
      if(typeof window.kpssOpenFocus==='function') return window.kpssOpenFocus();
      const hub=document.querySelector('#hubFocusBtn'); if(hub) return hub.click();
      toast('Odak oturumu hazırlanıyor.');
    });
  ...
  function paketIstekleri(count,subject,difficulty){
    const dersler=subject==='tumu'?SUBJECTS_META:SUBJECTS_META.filter(d=>d.id===subject);
    const kaynak=dersler.length?dersler:SUBJECTS_META;
    const paketler=[];
    let kalan=count, i=0;
    while(kalan>0){
      const d=kaynak[i%kaynak.length];
      const topics=TOPICS_SEED[d.id]||[d.name];
      const topic=topics[Math.floor(Math.random()*topics.length)];
      const n=Math.min(5,kalan);
      paketler.push({subject:d.name,topic,difficulty,count:n});
      kalan-=n;i++;
    }
    return paketler;
  }
  ...
  async function smartTestModal(){
    ...
    modalAc('⚡ Akıllı Test',`<div class="field"><label>Ders</label><select id="smartSubject"><option value="tumu">Tüm dersler</option>${dersler.map(d=>`<option value="${d.id}">${esc(d.name)}</option>`).join('')}</select></div><div class="field"><label>Zorluk</label><select id="smartDifficulty"><option>orta</option><option>kolay</option><option>zor</option></select></div><p class="alt">${testCount} soru hazırlanacak. Sistem backend'in kullandığı <b>generate-mixed-test</b> uç noktasını kullanır ve soruları 5'li paketler halinde ister.</p>`,'<button type="button" class="btn btn-accent" id="smartGo">Soruları Getir</button>');
    $("#smartGo").onclick=async()=>{
      const subject=$("#smartSubject").value;const difficulty=$("#smartDifficulty").value;const paketler=paketIstekleri(testCount,subject,difficulty);let started=Date.now();
      modalAc('⏳ Test hazırlanıyor',`<div class="ai-progress"><strong id="aiProgressTitle">${paketler.length} paket hazırlanıyor…</strong><p id="aiProgressText">Backend AI servisinden gerçek sorular bekleniyor.</p><div class="ai-progress-bar"><i id="aiProgressBar"></i></div><small id="aiProgressTime">Geçen süre: 0 sn</small></div>`,'<button type="button" class="btn btn-outline" id="aiCancelBtn">İptal</button>');
      const timeTick=setInterval(()=>{const e=Math.floor((Date.now()-started)/1000);const el=document.querySelector('#aiProgressTime');if(el)el.textContent=`Geçen süre: ${e} sn`;},1000);
      try{
        const r=await apiKarisikTestOlustur(paketler);
        clearInterval(timeTick);
        if(!r.ok){modalAc('Test oluşturulamadı',`<p>${esc(r.mesaj)}</p><p class="alt">İstek backend'e ulaştı ancak geçerli soru paketi dönmedi. Tekrar deneyebilirsin.</p>`,'<button type="button" class="btn btn-accent" id="smartClose">Kapat</button>');$("#smartClose").onclick=modalKapat;return;}
        const qs=r.veri?.sorular||r.veri?.questions||[];
        if(qs.length!==testCount){modalAc('Eksik soru döndü',`<p>İstenen ${testCount} soru yerine ${qs.length} soru geldi. Testi eksik başlatmadım.</p>`,'<button type="button" class="btn btn-accent" id="smartClose">Kapat</button>');$("#smartClose").onclick=modalKapat;return;}
        modalKapat();testModalBaslat(qs,`${subject==='tumu'?'Karma':dersAdi(subject)} — ${testCount} Soruluk Akıllı Test`);
      }catch(err){clearInterval(timeTick);modalAc('Test oluşturulamadı',`<p>${esc(err?.message||'Beklenmeyen bir hata oluştu.')}</p>`,'<button type="button" class="btn btn-accent" id="smartClose">Kapat</button>');$("#smartClose").onclick=modalKapat;}
    };
  }
