...
async function apiIstek(yol,{method="GET",body,timeoutMs=25000}={}){const baseUrl=apiBaseUrlAl();if(!navigator.onLine)return{ok:false,cozumsuz:true,mesaj:"İnternet bağlantısı gerekiyor."};const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{const res=await fetch(baseUrl+yol,{method,headers:{"Content-Type":"application/json","X-User-Id":cihazKullaniciId()},body:body?JSON.stringify(body):undefined,signal:controller.signal,cache:"no-store"});const veri=await res.json().catch(()=>({}));if(!res.ok)return{ok:false,status:res.status,mesaj:veri.hata||veri.error||`Sunucu ${res.status} döndürdü.`,yol,baseUrl};return{ok:true,veri}}catch(err){if(err.name==="AbortError")return{ok:false,mesaj:"İstek zaman aşımına uğradı. Sunucu belirtilen sürede yanıt vermedi."};return{ok:false,mesaj:"Sunucuya şu anda ulaşılamıyor. Lütfen tekrar deneyin."}}finally{clearTimeout(timer)}}
async function apiBaglantiTesti(){return apiIstek("/health",{timeoutMs:10000})}
async function apiAiOgretmenSor(soru){return apiIstek("/api/ai/teacher",{method:"POST",body:{soru},timeoutMs:120000})}
...
async function apiKarisikTestOlustur(istekler){const paketSayisi=Array.isArray(istekler)?istekler.length:1;const timeout=Math.min(900000,Math.max(150000,paketSayisi*90000));return apiIstek("/api/ai/generate-mixed-test",{method:"POST",body:{istekler},timeoutMs:timeout})}
async function apiGercekDenemeOlustur(istekler){const paketSayisi=Array.isArray(istekler)?istekler.length:1;const timeout=Math.min(900000,Math.max(180000,paketSayisi*100000));return apiIstek("/api/ai/generate-mock-exam",{method:"POST",body:{istekler},timeoutMs:timeout})}
